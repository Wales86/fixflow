...
*   _F-001_: Możliwość tworzenia, przeglądania, aktualizowania i usuwania profili klientów w cyfrowej bazie danych.
...
*   _F-008_: Mechanicy mogą wybrać swoje imię i aktywne zlecenie z listy, aby zarejestrować czas pracy.
...
*   _F-013_: System musi wspierać trzy role użytkowników: Właściciel, Biuro, Mechanik, z predefiniowanymi uprawnieniami.
...
*   _Opis_: Jako zarejestrowany użytkownik (Właściciel lub pracownik Biura), chcę mieć możliwość bezpiecznego zalogowania się do aplikacji, aby uzyskać dostęp do jej funkcji zgodnie z moją rolą. Mechanicy będą korzystać z jednego wspólnego konta bez konieczności logowania - wybierają swoje imię z listy przy rejestracji czasu.
*   _Kryteria akceptacji_:
    *   Będąc na stronie logowania, widzę pola na nazwę użytkownika i hasło.
    *   Gdy wpiszę poprawne dane uwierzytelniające i prześlę formularz, zostaję przekierowany do głównego panelu.
...
*   _Opis_: Jako Właściciel lub pracownik Biura, chcę dodać nowego klienta do systemu, podając jego dane (w tym imię, nazwisko, telefon, email, adres), abym mógł tworzyć dla niego zlecenia naprawy.
*   _Kryteria akceptacji_:
...
*   _Opis_: Jako Właściciel lub pracownik Biura, chcę przeglądać listę wszystkich klientów, abym mógł szybko znaleźć i zarządzać ich informacjami.
*   _Kryteria akceptacji_:
    *   Będąc zalogowanym jako Właściciel lub pracownik Biura, mam dostęp do strony, która wyświetla przeszukiwalną i sortowalną listę wszystkich klientów.
...
*   _Opis_: Jako Właściciel lub pracownik Biura, chcę dodać nowy pojazd do profilu klienta, w tym jego Markę, Model, rocznik, VIN i numer rejestracyjny, abym mógł śledzić jego historię serwisową.
*   _Kryteria akceptacji_:
    ...
    *   System powinien uniemożliwić dodanie pojazdu z numerem VIN, który już istnieje w systemie.
...
*   _Opis_: Jako użytkownik, chcę mieć możliwość szybkiego wyszukania pojazdu po numerze rejestracyjnym, VIN, marce lub modelu, aby sprawnie odnaleźć jego dane i historię.
*   _Kryteria akceptacji_:
...
*   _Opis_: Jako użytkownik, chcę widzieć listę wszystkich aktywnych (nie "Zamkniętych") zleceń naprawy, aby mieć ogólny obraz bieżącego obciążenia pracą.
*   _Kryteria akceptacji_:
    *   Będąc zalogowanym, mogę przejść do panelu, który pokazuje listę wszystkich zleceń o statusie innym niż "Zamknięte".
...
*   _Kryteria akceptacji_:
    *   Będąc zalogowanym jako Mechanik, mogę wybrać aktywne zlecenie naprawy.
    *   Następnie mogę wybrać swoje imię z listy pracowników.
    *   Mogę wprowadzić czas spędzony (np. w godzinach i minutach) oraz opis wykonanej pracy.
...
*   _Opis_: Jako Właściciel, chcę wygenerować raport dla konkretnego mechanika w danym zakresie dat (dzień, tydzień, miesiąc), aby zobaczyć wszystkie zadania, nad którymi pracował, i całkowity czas, który zarejestrował.
*   _Kryteria akceptacji_:
    *   Będąc na stronie raportowania, mogę wybrać pracownika i zakres dat.
...
*   _Opis_: Jako Właściciel, chcę wygenerować raport dla całego zespołu w danym zakresie dat, aby porównać ich czasy pracy i zobaczyć ogólną produktywność.
*   _Kryteria akceptacji_:
    *   Będąc na stronie raportowania, mogę wybrać opcję raportu dla całego zespołu i wybrać zakres dat.
    *   Raport wyświetla tabelę z listą mechaników i sumą ich zarejestrowanego czasu w danym okresie.
