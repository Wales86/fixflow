...
## Enums
- Typically, keys in an Enum should be TitleCase. For example: `FavoritePerson`, `BestLake`, `Monthly`. Generate Enums always in the folder `app/Enums`
...
- Use Request classes for http validation and spatie/laravel-data for DTO and types validation. Example:
```
...
- Use Laravel's built-in authentication and authorization features (gates, policies, Sanctum, etc.). Use spate/laravel-permission for authorization with Laravel built-in functions.
